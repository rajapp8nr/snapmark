...
func (o EllipseOp) Apply(dst draw.Image) {
	x1, y1, x2, y2 := norm(o.X1, o.Y1, o.X2, o.Y2)
	rx := float64(max(1, (x2-x1)/2))
	ry := float64(max(1, (y2-y1)/2))
	cx := float64(x1) + rx
	cy := float64(y1) + ry
	for t := 0.0; t < 2*math.Pi; t += 0.01 {
		x := int(cx + rx*math.Cos(t))
		y := int(cy + ry*math.Sin(t))
		for i := 0; i < o.Stroke; i++ {
			setPixel(dst, x+i, y, o.Color)
		}
	}
}
...
func norm(x1, y1, x2, y2 int) (int, int, int, int) {
	if x2 < x1 {
		x1, x2 = x2, x1
	}
	if y2 < y1 {
...
func drawLine(img draw.Image, x1, y1, x2, y2 int, c color.Color, stroke int) {
	...
	for {
		for i := 0; i < stroke; i++ {
			setPixel(img, x1+i, y1, c)
			setPixel(img, x1, y1+i, c)
		}
		if x1 == x2 && y1 == y2 {
			break
		}
